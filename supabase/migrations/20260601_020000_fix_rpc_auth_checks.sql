-- security: add caller-membership auth check to record_manual_payment
-- and mark_org_setup_step; revoke anon EXECUTE.
--
-- Audit findings (full-recon HIGH, not in original docs/rls-fix-plan.md):
--   record_manual_payment was SECURITY DEFINER, executable by anon,
--   and validated (order_id, org_id) but never checked auth.uid() is a
--   member of p_org_id. Any anon caller could insert fabricated
--   payments or refunds against any order in any org and rewrite
--   orders.balance_due_amount.
--
--   mark_org_setup_step was SECURITY DEFINER, executable by anon, with
--   no auth check at all. Any caller could flip arbitrary
--   settings.setup_progress.* keys on any org.

-- Re-define record_manual_payment with a membership guard at the top.
...
begin
  -- Caller-membership check: prevent anon (and cross-org) callers from
  -- fabricating payments. Server actions still pre-check role; this is
...
-- Re-define mark_org_setup_step with an EXISTS guard in the WHERE clause.
-- Silent no-op for unauthorized callers preserves the existing
-- non-blocking semantics in lib/guidance/update-setup-progress.ts.
create or replace function public.mark_org_setup_step(p_org_id uuid, p_step text)
returns void
language sql
security definer
set search_path to 'public'
as $function$
  ...
  set settings = jsonb_set(
    jsonb_set(
      ...
      '{setup_progress}',
      coalesce((o.settings::jsonb -> 'setup_progress'), '{}')
    ),
...
-- Revoke anon EXECUTE; only authenticated (and service_role) needed.
revoke execute on function public.record_manual_payment(
  uuid, uuid, numeric, text, text, text, timestamptz
) from anon;
revoke execute on function public.mark_org_setup_step(uuid, text) from anon;
