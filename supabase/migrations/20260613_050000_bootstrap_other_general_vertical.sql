-- Wave 2 follow-up — "Other / general rentals" signup vertical.
--
-- Adds an `other` branch to bootstrap_organization so an operator who
-- picks the setup-only catch-all at signup is seeded with neutral,
-- rename-friendly starter categories instead of falling through to the
-- inflatable bounce-house seed. Category names mirror
-- lib/verticals/other.ts defaultCategorySeeds so signup and the
-- add-vertical flow agree.
--
-- Everything else (existing per-vertical branches, the inflatable
-- fallback for unknown types, organization_verticals dual-write) is
-- preserved verbatim from 20260608_170000.
...
begin
  ...
  if v_type = 'car' then
    ...
    -- Neutral starters for a generalist — mirror
    -- lib/verticals/other.ts defaultCategorySeeds. The operator renames
    -- these to match their inventory.
    insert into categories (organization_id, name, slug, sort_order, vertical) values
      (v_org_id, 'Featured Items', 'featured-items', 1, 'other'),
      (v_org_id, 'Equipment',      'equipment',      2, 'other'),
      (v_org_id, 'Accessories',    'accessories',    3, 'other'),
      (v_org_id, 'Add-Ons',        'add-ons',        4, 'other');
