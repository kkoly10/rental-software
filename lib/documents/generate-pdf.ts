...
const RENTAL_AGREEMENT_TERMS = [
  ...
  "4. WEATHER POLICY: Inflatables must not be used in winds exceeding 15 mph, rain, or lightning. Customer may reschedule once at no charge if weather conditions are dangerous.",
  "5. SUPERVISION: An adult 18+ must supervise the equipment at all times. The renter assumes all responsibility for safe use.",
  "6. CAPACITY & USE: Equipment must not exceed posted weight/capacity limits. No shoes, sharp objects, food, or silly string on inflatables. Violating these rules may result in immediate removal with no refund.",
...
const SAFETY_WAIVER_TERMS = [
  "1. ASSUMPTION OF RISK: I understand that the use of inflatable and party equipment involves risks, including but not limited to falls, collisions, and entrapment. I voluntarily assume all such risks.",
  ...
  "3. SAFETY RULES: I agree to enforce all posted safety rules, including no shoes, no rough play, no food or drinks inside inflatables, no silly string, and maintaining capacity limits at all times.",
...
export function generateDocumentPdf(data: DocumentPdfData): Uint8Array {
  ...
  doc.setTextColor(85, 112, 143);
  doc.text(
    `${data.businessName} · ${data.supportEmail} · Generated by Korent`,
    pageWidth / 2,
    footerY,
    { align: "center" }
  );
