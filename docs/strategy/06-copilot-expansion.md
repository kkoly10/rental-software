...
- **Hardening (implemented):**
  ...
  2. **Hash-chained, WORM `app_event_logs`** (`supabase/migrations/20260605_020000_*`): a `BEFORE INSERT` trigger assigns a sequence and a SHA-256 `entry_hash` covering the previous row's hash (tamper-EVIDENT — any edit/insert/delete breaks the chain), and `BEFORE UPDATE/DELETE` triggers block mutation entirely (append-only, even for the service role). `verify_app_event_log_chain()` walks the chain and returns the first break. `app_event_logs` was verified INSERT-only in the codebase, so WORM breaks nothing.
...
- **Deployment note:** the two migrations are **not run by CI/deploy** — an owner must apply them to the Supabase project for the WORM chain and the acknowledgment gate to activate. Until then the app behaves exactly as before (chain dormant; ack gate fails open).
