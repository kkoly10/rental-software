...
| **Refined vertical scope: inflatables ONLY for launch, vehicles & fleet queued as #2** | Jun 2026 | 12-vertical capability audit + US demand-vs-competition research showed (a) Korent is 10/10 ready for inflatables and the incumbents (InflatableOffice, ERS) are dated 2010s software, (b) vehicles & fleet has the fastest growth (+11.6% CAGR P2P) AND the best architectural fit with Korent's existing delivery/pickup/photo/signature stack at ~5 engineer-days build cost. Tents/wedding events deferred (12+ saturated SaaS competitors, Goodshuffle raised $5M). Watercraft deferred despite highest search demand (5-month seasonality + state-by-state marine insurance regulatory burden too heavy for solo founder). | [05](./05-vertical-roadmap.md) |
| **🔄 SUPERSEDES THE ROW ABOVE — Expand within party umbrella (5 sub-verticals), defer vehicles & fleet** | Jun 5, 2026 | Competitor recon (Goodshuffle Pro / TapGoods Pro / Booqable / InflatableOffice) confirmed all serious party-rental SaaS uses dedicated landing pages per sub-vertical, and the existing inflatable customer base overwhelmingly also rents tents, tables/chairs, dance floors, photo booths, and concessions. Expanding *within* the party umbrella captures more of the same operator's wallet at lower architectural cost than pivoting to vehicles. Adopting a **capabilities-based architecture** so each future vertical = a thin config + content + images, not a copy-pasted module. Authoritative source-of-truth: [`docs/architecture/multi-vertical-capabilities.md`](../architecture/multi-vertical-capabilities.md). | [05 amendment](./05-vertical-roadmap.md), [architecture doc](../architecture/multi-vertical-capabilities.md) |
...
| **QBO CSV export added to Sprint 1, Xero added to Phase 2** | Jun 2026 | CSV export is a 1-day quick-win that removes the immediate "do you sync with QuickBooks?" sales objection before the full integration ships. Xero leapfrogs Goodshuffle (no Xero) and matches Booqable's beta — same integration shape as QBO so cheap to add | [Master plan](../../COMPETITIVE_POSITIONING_MASTER_PLAN.md) Sprint 1 & 3.5 |
| **Smart Delivery Mode (Option C): "route" is invisible by default, Settings toggle for manual mode** | Jun 2026 | Recon of the existing delivery flow found a 3-click ritual ("create route → add stop → start delivery") with jargon a noob doesn't recognize. ~80-90% of US/Mexico party rental operators are not on any rental SaaS today, so the median Korent buyer has never used the word "route" professionally. Onboarding-time choice rejected as a SaaS anti-pattern (forces uninformed decision, doubles maintenance, anchors wrong mental model). Ship one good default + Settings → Advanced toggle for the power-user minority. | [Master plan](../../COMPETITIVE_POSITIONING_MASTER_PLAN.md) Sprint 1.5 |
