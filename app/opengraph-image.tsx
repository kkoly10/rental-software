...
function gradientFor(primary: string): string {
  if (!primary) return KORENT_GRADIENT;
  // Normalize 3-char shorthand (#abc → #aabbcc) before building the gradient.
  const hex3 = primary.match(/^#([0-9a-fA-F])([0-9a-fA-F])([0-9a-fA-F])$/);
  const normalized = hex3
    ? `#${hex3[1]}${hex3[1]}${hex3[2]}${hex3[2]}${hex3[3]}${hex3[3]}`
    : primary;
  if (!/^#[0-9a-fA-F]{6}$/.test(normalized)) return KORENT_GRADIENT;
  return `linear-gradient(135deg, ${normalized} 0%, ${normalized}cc 100%)`;
}
...
export default async function OgImage() {
  ...
  if (!tenant) {
    return new ImageResponse(korentImage, { ...size });
  }
