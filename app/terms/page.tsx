...
import { StorefrontTermsBody } from "@/components/public/legal/storefront-legal";
...
export async function generateMetadata(): Promise<Metadata> {
  if (await isTenantHost()) {
    ...
    return await buildPageMetadata({
      title: `Rental Terms — ${name}`,
      description: `The terms that govern rentals booked from ${name}.`,
      path: "/terms",
    });
  }
  return await buildPageMetadata({
...
export default async function TermsPage() {
  ...
  // On an operator storefront, show the operator's rental terms — their own
  // externally-hosted page (redirect) or Korent's storefront baseline copy.
  // Korent's SaaS Terms of Service (below) govern the operator↔Korent
  // subscription and are only correct on the root domain.
  if (await isTenantHost()) {
    const settings = await getOrganizationSettings();
    if (isAbsoluteHttpUrl(settings.legalTermsUrl)) {
      redirect(settings.legalTermsUrl);
    }
